...
pub fn start(shared: SharedState, socket: UdpSocket) -> io::Result<(CommandSender, TuiReceiver)> {
  ...
  thread::spawn(switchboard(shared.clone(), snooze_tx, gig_tx, socket, reciever, sockets.clone(), tui_tx.clone()));
  thread::spawn(lifetime(shared.clone(), snooze_rx, statuses.clone(), tui_tx.clone()));
